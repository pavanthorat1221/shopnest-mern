...
const AdminOrders = () => {
  ...
  const updateStatus = async (id, status) => {
    ...
    try {
      ...
      setOrders((currentOrders) => currentOrders.map((order) => order._id === id ? { ...order, status: data.status } : order));
    } catch (error) {
  ...
  return (
    <div style={containerStyle}>
      ...
      {statusError && <p style={{ color: '#fca5a5', marginBottom: '15px' }}>{statusError}</p>}
