...
const Cart = () => {
  ...
  const handleUpdateQty = (item, qty) => {
    if (qty > 0 && (!Number.isFinite(item.stock) || qty <= item.stock)) {
              ...
              <div key={item.productId} className="cart-item">
                ...
                <div className="cart-item-details">
                  ...
                  <div className="qty-controls">
                    ...
                    <span>{item.qty}</span>
                    <button onClick={() => handleUpdateQty(item, item.qty + 1)} disabled={Number.isFinite(item.stock) && item.qty >= item.stock}>+</button>
                  </div>
                  {Number.isFinite(item.stock) && <small className="stock-note">{item.stock} available</small>}
